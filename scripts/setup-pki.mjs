#!/usr/bin/env node
/**
 * PKI setup script for Naylence Advanced Security examples.
 *
 * Generates a root CA, intermediate CA, and supporting configuration using
 * the TypeScript implementation of Naylence's internal CA service.
 */
...
import {
  CASigningService,
  createTestCA,
} from "../dist/esm/naylence/fame/security/cert/internal-ca-service.js";
...
async function setupPKI({ output, org, logicals }) {
  ...
  console.log(`🏗️ Generating PKI hierarchy in ${outputDir}`);
  ...
  const signingCertPath = intermediateCertPath;
  const signingKeyPath = intermediateKeyPath;

  const envPath = join(outputDir, "fame-ca.env");
  const envContent = `# Generated by setup-pki.mjs
...
export FAME_SIGNING_CERT_FILE="${signingCertPath}"
export FAME_SIGNING_KEY_FILE="${signingKeyPath}"
...
`;
  ...
  const readme = `# Naylence PKI Setup - ${org}
...
${normalizedLogicals.map((logical) => `- ${logical}`).join("\n")}

`; 

  await writeFileUtf8(readmePath, readme);

  console.log("🎉 PKI setup complete!");
  console.log(`📄 Root CA certificate: ${rootCertPath}`);
  console.log(`🔑 Root CA key:        ${rootKeyPath}`);
  console.log(`📄 Intermediate cert:  ${intermediateCertPath}`);
...
if (process.cwd() !== scriptDir) {
  console.log(`ℹ️  Using script from ${scriptDir}`);
}
