...
const expectedFactories = [
  { base: DEFAULT_CA_SERVICE_FACTORY_META.base, key: DEFAULT_CA_SERVICE_FACTORY_META.key },
  { base: DEFAULT_CERTIFICATE_MANAGER_FACTORY_META.base, key: DEFAULT_CERTIFICATE_MANAGER_FACTORY_META.key },
  { base: CHANNEL_ENCRYPTION_FACTORY_META.base, key: CHANNEL_ENCRYPTION_FACTORY_META.key },
  { base: COMPOSITE_ENCRYPTION_FACTORY_META.base, key: COMPOSITE_ENCRYPTION_FACTORY_META.key },
  { base: DEFAULT_SECURE_CHANNEL_FACTORY_META.base, key: DEFAULT_SECURE_CHANNEL_FACTORY_META.key },
  { base: X25519_ENCRYPTION_FACTORY_META.base, key: X25519_ENCRYPTION_FACTORY_META.key },
  { base: X5C_KEY_MANAGER_FACTORY_META.base, key: X5C_KEY_MANAGER_FACTORY_META.key },
  { base: AFT_LOAD_BALANCER_FACTORY_META.base, key: AFT_LOAD_BALANCER_FACTORY_META.key },
  { base: AFT_REPLICA_FACTORY_META.base, key: AFT_REPLICA_FACTORY_META.key },
  { base: ADVANCED_WELCOME_FACTORY_META.base, key: ADVANCED_WELCOME_FACTORY_META.key },
];
...
describe("advanced security plugin", () => {
  ...
  it("registers advanced security and supplemental factories", async () => {
    ...
    expect(registrar.registerFactory).toHaveBeenCalledTimes(expectedFactories.length);
    for (const expected of expectedFactories) {
      expect(registrations).toEqual(expect.arrayContaining([expect.objectContaining(expected)]));
    }
  });
  ...
  it("exposes the plugin specifier", async () => {
    const pluginModule = await import("../plugin.js");
    expect(pluginModule.ADVANCED_SECURITY_PLUGIN_SPECIFIER).toBe(pluginModule.default.name);
  });
});
