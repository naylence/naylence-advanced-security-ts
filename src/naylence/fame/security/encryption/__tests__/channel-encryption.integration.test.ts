...
describe("ChannelEncryptionManager integration", () => {
  it("performs a full secure-channel handshake and round-trips payload encryption", async () => {
    ...
    expect(dataDelivery.envelope.sec?.enc?.alg).toBe(
    ...
    const decryptedEnvelope =
      await receiverManager.decryptEnvelope(inboundEnvelope);

    expect(decryptedEnvelope.frame?.codec).toBe("json");
    expect((decryptedEnvelope.frame as { payload?: unknown })?.payload).toEqual(
      envelopePayload,
    );
  ...
  it("queues the first send, then encrypts and delivers subsequent channel messages", async () => {
    ...
    const decryptedQueued =
      await receiverManager.decryptEnvelope(queuedEnvelope);
    expect((decryptedQueued.frame as { payload?: unknown })?.payload).toEqual(
      firstPayload,
    );
    ...
    if (immediateEnvelope.sec?.enc?.val) {
      ...
      immediateEnvelope.sec.enc.val =
        Buffer.from(nonceBytes).toString("base64");
    }

    const decryptedImmediate =
      await receiverManager.decryptEnvelope(immediateEnvelope);
    expect(decryptedImmediate.frame?.codec).toBe("json");
    expect(
      (decryptedImmediate.frame as { payload?: unknown })?.payload,
    ).toEqual(secondPayload);
