...
class RecordingSealedEncryptionManager implements EncryptionManager {
	...
	public readonly encryptCalls: Array<{ envelope: FameEnvelope; opts?: EncryptionOptions }>; 
	public readonly decryptCalls: Array<{ envelope: FameEnvelope; opts?: EncryptionOptions }>; 
...
class RecordingChannelEncryptionManager extends RecordingSealedEncryptionManager {
	...
	async notifyChannelFailed?(channelId: string, reason?: string) {
		this.channelFailed.push(reason === undefined ? { channelId } : { channelId, reason });
	}
}
...
abstract class BaseTestFactory extends EncryptionManagerFactory<EncryptionManagerConfig> {
	...
	constructor(public readonly type: string, public readonly priority = 100) {
		super();
	}
...
class TestSealedFactory extends BaseTestFactory {
	...
	public override supportsOptions(opts?: EncryptionOptions | null): boolean {
		...
		return (
			candidate?.encryptionType === "sealed" || candidate?.encryption_type === "sealed"
		);
	}
...
class TestChannelFactory extends BaseTestFactory {
	...
	public override supportsOptions(opts?: EncryptionOptions | null): boolean {
		const candidate = opts as EncryptionOptions & { encryption_type?: string };
		return candidate?.encryptionType === "channel" || candidate?.encryption_type === "channel";
	}
...
class TestAltSealedFactory extends BaseTestFactory {
	...
	public override supportsOptions(opts?: EncryptionOptions | null): boolean {
		const candidate = opts as EncryptionOptions & { encryption_type?: string };
		return (
			candidate?.encryptionType === "sealed-alt" || candidate?.encryption_type === "sealed-alt"
		);
	}
...
class TestCompositeEncryptionManager extends CompositeEncryptionManager {
	constructor(deps: CompositeEncryptionManagerDependencies, registry: FakeRegistry) {
		super(deps);
...
function createDependencies(overrides: Partial<CompositeEncryptionManagerDependencies> = {}): CompositeEncryptionManagerDependencies {
	return {
...
describe("CompositeEncryptionManager behavior", () => {
	...
	beforeEach(() => {
		...
		composite = new TestCompositeEncryptionManager(createDependencies(), registryStub);
	});

	it("skips encryption when no matching manager exists", async () => {
		const envelope = createEnvelope();
		const result = await composite.encryptEnvelope(envelope, { encryptionType: "unknown" });
	...
	it("delegates encryption to matching manager based on options", async () => {
		const envelope = createEnvelope();
		const result = await composite.encryptEnvelope(envelope, { encryptionType: "sealed" });
	...
	it("forwards channel notifications to existing manager", async () => {
		await composite.encryptEnvelope(createEnvelope(), { encryptionType: "channel" });
		const channelManager = channelFactory.createdManagers[0] as RecordingChannelEncryptionManager;
		...
		expect(channelManager.channelFailed).toEqual([{ channelId: "channel-42", reason: "timeout" }]);
	});

	it("forwards key availability notifications to sealed manager", async () => {
		await composite.encryptEnvelope(createEnvelope(), { encryptionType: "sealed" });
	...
	it("initializes default managers and propagates node events", async () => {
		...
		expect(sealedManager.upstreamAttached).toEqual([{ node, info: attachInfo }]);
	...
	it("applies node context to managers created after node start", async () => {
		...
		await composite.encryptEnvelope(createEnvelope(), { encryptionType: "sealed-alt" });
