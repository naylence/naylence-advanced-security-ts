...
type FactoryConstructor = new (...args: unknown[]) => ResourceFactory<unknown, unknown>;
...
async function registerModule(
  ...
  for (const candidate of candidates) {
    try {
      ...
      const meta = (mod as Record<string, unknown>).FACTORY_META as FactoryMetadata | undefined;
      const Ctor = (mod as Record<string, unknown>).default as FactoryConstructor | undefined;

      if (!meta?.base || !meta?.key || typeof Ctor !== "function") {
        console.warn("[debug] invalid factory module", spec, { meta, hasCtor: typeof Ctor === "function" });
        console.warn(
      ...
      if (!moduleNotFound || isLastCandidate) {
        console.warn("[debug] failed to import candidate", { spec, candidate, message });
